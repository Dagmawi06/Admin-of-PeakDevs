...
exports.handler = async (event) => {
  ...
  try {
    const limitParam = Number.parseInt(event.queryStringParameters?.limit || "50", 10);
    const offsetParam = Number.parseInt(event.queryStringParameters?.offset || "0", 10);
    ...
    if (error) {
      return json(500, { ok: false, error: error.message || "Failed to fetch messages." });
    }

    const rows = Array.isArray(data) ? data : [];
    const nextOffset = rows.length === limit ? offset + rows.length : null;
    return json(200, { ok: true, data: rows, nextOffset });
  } catch (err) {
